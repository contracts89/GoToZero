...
Objects: 
Х	Background 
Х	Background menu Ц background image for menu
Х	Background game Ц background image for game
1.1.	Menu
The Main menu gives the player the following options:
Start -  starts the game
Help Ц describes the goal of the game
HighScore Ц shows high-scores(TO DO)
Quit Ц exits the game
1.2.	Falling objects
Falling object number Ц falling numbers with random value
Falling object sign Ц falling mathematic signs corresponding to the mathematical operations of addition, subtraction, multiplication and division(TO DO)
Falling object symbol Ц falling symbols (like $, %, #) which have an unpredictable outcome for the score(TO DO)
1.3.	Main Player
Object controlled by the player, representing a female ninja who runs side to side and collects the falling objects 
1.4.	Static Objects
Score Ц object, which shows current score
Timer Ц object, which shows time elapsed since the game has begun(TO DO)
1.5.	Other Objects
Pause Ц the player can pause the game at any time with the button Pause(TO DO)
Exit Ц the player can exit the game at any time with the button Exit(TO DO)
...
1.6.	»збор на бутон Start
	...
	- each object has its own individual УfallФ lane
...
1.7.	End Of The Game
	...
	- the playerТs score is compared to the High Scores(TO DO)
